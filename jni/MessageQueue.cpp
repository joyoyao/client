...
namespace openamedia {
	...
	MessageQueue::MessageQueue(void (*handleMessage)(Message* msg, void* context), void (*threadExit)(void* context), void* context)
		:mDone(false),
		 mThreadExited(false),
		 mStarted(false),
		 mHandleMessage(handleMessage),
		 mThreadExitFnc(threadExit),
		 mContext(context){
		start();
	}
